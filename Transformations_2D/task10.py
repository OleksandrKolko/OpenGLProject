import sys
import numpy as np

sys.path.append('src')

from engine.scene.Scene import Scene
from engine.model.Polygon import Polygon

FIGURE_KEY = "square"


class Task10Scene(Scene):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        square = Polygon(
            0, 0,
            1, 0,
            1, 1,
            0, 1,
            color="gray",
            linewidth=2,
            line_style="solid",
            vertices_show=True,
            vertex_color="black",
            vertex_size=30
        )

        self[FIGURE_KEY] = square


def initial_frame(scene: Scene):
    square: Polygon = scene[FIGURE_KEY]
    square["color"] = "gray"
    square["line_style"] = "solid"
    square.rotation = 0
    square.translation = (0, 0)
    square.scale = (1, 1)


def order1_frame(scene: Scene):
    square: Polygon = scene[FIGURE_KEY]
    square["color"] = "red"
    square["line_style"] = "solid"

    square.pivot(0.5, 0.5)
    square.scale = (2, 2)
    square.rotation = np.radians(30)
    square.translation = (1, -1)


def order2_frame(scene: Scene):
    square: Polygon = scene[FIGURE_KEY]
    square["color"] = "blue"
    square["line_style"] = "solid"

    square.pivot(0.5, 0.5)
    square.translation = (1, -1)
    square.scale = (2, 2)
    square.rotation = np.radians(30)


def order3_frame(scene: Scene):
    square: Polygon = scene[FIGURE_KEY]
    square["color"] = "green"
    square["line_style"] = "solid"

    square.pivot(0.5, 0.5)
    square.scale = (2, 2)
    square.translation = (1, -1)
    square.rotation = np.radians(30)


scene = Task10Scene(
    image_size=(10, 8),
    coordinate_rect=(-1, -3, 6, 3),
    title="Task 10: Three Orders of Transformations",
    axis_show=True,
    grid_show=True,
    axis_color=("red", "green"),
    base_axis_show=False
)

scene.add_frames(initial_frame, order1_frame, order2_frame, order3_frame)
scene.show()